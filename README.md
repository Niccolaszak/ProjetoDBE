...
Conforme solicitado no enunciado, o foco do projeto foi a aplicação correta e justificada de padrões de arquitetura e projeto.
...
* **Onde:** As permissões são carregadas dinamicamente no `app/Providers/AuthServiceProvider.php`. As regras de acesso para cada modelo (ex: `Livro`) são definidas em suas respectivas *Policies* (ex: `app/Policies/LivroPolicy.php`).
* **Justificativa:** Centraliza o controle de acesso, limpa os controllers (que agora usam `$this->authorizeResource(...)`) e segue as melhores práticas de segurança do framework.
...
* **Justificativa:** Aplica o **Princípio da Inversão de Dependência (D do SOLID)**. Isso desacopla nossa aplicação do Eloquent, facilitando a manutenção e, crucialmente, permitindo a criação de testes unitários "mockados".
...
* **Justificativa:** Aplica o **Princípio da Responsabilidade Única (S do SOLID)**. Nossos controllers agora são "magros" (*thin controllers*); eles apenas orquestram as requisições HTTP e delegam o trabalho para os serviços.

### 4. Padrões Strategy e Factory Method (Fase 3)

Aplicamos os padrões **Strategy** e **Factory Method** para gerenciar a lógica complexa de criação de movimentações de estoque.
...
* **Justificativa:** O `MovimentacaoService` agora obedece ao **Princípio Aberto/Fechado (O do SOLID)**. Podemos adicionar novos tipos de movimentação (como 'Devolução' ou 'Ajuste') apenas criando uma nova classe Strategy, sem nunca precisar alterar o `MovimentacaoService`.

### 5. CQRS (Command Query Responsibility Segregation) (Fase 4)

Refatoramos o `LivroController` para aplicar o padrão CQRS, separando completamente as operações de leitura (Queries) das de escrita (Commands).
...
* **Justificativa:** O controller ficou extremamente limpo e focado. Este padrão permite que as lógicas de leitura sejam otimizadas de forma independente das lógicas de escrita, melhorando a manutenção e a performance do sistema.

---

## Testes Automatizados

Para garantir a qualidade e a funcionalidade da nossa lógica de negócio, criamos testes unitários.
